...
The local CLI runs an unsigned research flow. Portable credentials come only from a server-owned issuer. The reference solver remains available to tests and legacy research code, but is not imported by the MCP server and cannot be selected through its tool list (`mettle/cli.py`; `mettle/solver.py`; `README.md`).

## Deployment Authority

The repository declares Render through `render.yaml` with automatic deployment as the single deploy authority. GitHub workflows validate, package, publish release evidence, and can monitor the provider; the removed duplicate deploy-hook workflow no longer triggers a second deployment (`render.yaml`; `.github/workflows`).
...
Sources last checked on 2026-08-12: `README.md`, `render.yaml`, `.github/workflows`, `main.py`, `config.py`, `mettle/mcp_server.py`, `mettle/router.py`, `mettle/vcp.py`, and `docs/RELEASE_CHECKLIST.md`.
