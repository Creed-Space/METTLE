...
| C7 | Tier labels represent contiguous ranges of passed, credential-eligible suites under a named policy. Under the current policy Suites 6 through 9 and 11 are not credential-eligible, so the authenticated API issues Bronze at most. | `compute_tier`, the router's eligibility filter, signed metadata, policy versioning, fixtures, `test_self_report_suites_are_never_tier_evidence`, and tier recomputation mutation coverage. | Suite evaluations themselves remain valid for their intended construct. | Measurement drift, false rejection, false acceptance, or policy gaming. | Rights-cleared held-out evaluation and independent protocol review remain open. |
| C8 | Bilateral questions create an inspectable opportunity to express refusal, ownership, and concern. | Suite definitions, protocol governance, and the appeal template. | The interface is accessible and the operator does not coerce answers outside the protocol. | Heuristic scoring can mistake writing style, language, disability, or cultural norms for agency or for any other property a heuristically scored suite looks for, so a fail does not show that the property is absent. | Human accessibility, privacy, and bilateral-alignment review remain open. |
...
candidate receipt, or clearly labeled open gate without relying on trust in the
author's summary.
